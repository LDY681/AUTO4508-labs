...
// #define DIST     360
// #define THRES    175
// #define CORRIDOR_WIDTH_MIN 100
// #define CORRIDOR_WIDTH_MAX 150
// #define CORRIDOR_WIDTH_INVALID 180
...
#define DIST     720
#define THRES    400
#define CORRIDOR_WIDTH_MIN 260
#define CORRIDOR_WIDTH_MAX 360
#define CORRIDOR_WIDTH_INVALID 400
...
/*
 * drive in smaller steps with angle correction
*/
void drive_w_correction() {
	for (int i = 0; i < DIST / DIST_CORR; ++i) {
    if (PSDGet(PSD_LEFT) < CORRIDOR_WIDTH_MIN || (PSDGet(PSD_RIGHT) < CORRIDOR_WIDTH_INVALID && PSDGet(PSD_RIGHT) > CORRIDOR_WIDTH_MAX)) {
      if (DEBUG) printf("CORR RIGHT, left: %d, right: %d\n", PSDGet(PSD_LEFT), PSDGet(PSD_RIGHT));
      VWTurn(-20, ASPEED);  // turn right
      VWWait();
      VWStraight(DIST_CORR, SPEED);
      VWWait();
      VWTurn(10, ASPEED);
      VWWait();
      if (++i == DIST / DIST_CORR) break;
    } else if (PSDGet(PSD_RIGHT) < CORRIDOR_WIDTH_MIN || (PSDGet(PSD_LEFT) < CORRIDOR_WIDTH_INVALID && PSDGet(PSD_LEFT) > CORRIDOR_WIDTH_MAX)) {
      if (DEBUG) printf("CORR LEFT, left: %d, right: %d\n", PSDGet(PSD_LEFT), PSDGet(PSD_RIGHT));
      VWTurn(20, ASPEED); // turn left
      VWWait();
      VWStraight(DIST_CORR, SPEED);
      VWWait();
      VWTurn(-10, ASPEED);
