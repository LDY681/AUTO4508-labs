...
void a_star(Node nodes[], int num_nodes)
{
    ...
    printf("nodes[start].x %d, nodes[start].y %d, nodes[goal].x %d, nodes[goal].y %d", nodes[start].x, nodes[start].y, nodes[goal].x, nodes[goal].y);
    ...
    while (open_size > 0)
    {
        ...
        for (int i = 0; i < nodes[current].num_neighbors; i++)
        {
            ...
            if (connection_matrix[current][neighbor] == -1)
                continue;
